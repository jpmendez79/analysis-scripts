...
void Brazil() {
  ...
  TLine *l2 = new TLine(xvals[qv[0]],0,xvals[qv[0]],max+10);
  l2->SetLineStyle(9);
  l2->SetLineColor(kOrange);
  l2->Draw("same");
  TLine *u2 = new TLine(xvals[qv[4]],0,xvals[qv[4]],max+10);
  u2->SetLineStyle(9);
  u2->SetLineColor(kOrange);
  u2->Draw("same");
  TLine *l1 = new TLine(xvals[qv[1]],0,xvals[qv[1]],max+10);
  l1->SetLineStyle(9);
  l1->SetLineColor(kGreen);
  l1->Draw("same");
  TLine *u1 = new TLine(xvals[qv[3]],0,xvals[qv[3]],max+10);
  u1->SetLineStyle(9);
  u1->SetLineColor(kGreen);
  u1->Draw("same");
  TLine *m = new TLine(xvals[qv[2]],0,xvals[qv[2]],max+10);
  m->SetLineStyle(9);
  m->SetLineColor(kRed);
  m->Draw("same");
  TLegend * leg = MakeLegend(0.65, 0.6, 0.9, 0.75);
  leg->Clear();
  leg->AddEntry(hsens,TString::Format("BNB Sensitivity #Deltam^{2}_{41}=%.1f", dm2_val),"l");
  leg->AddEntry(l2, TString::Format("#pm2#sigma = %1.3f/%1.3f",xvals[qv[4]],xvals[qv[0]]),"l");
  leg->AddEntry(l1,TString::Format("#pm1#sigma = %1.3f/%1.3f",xvals[qv[3]],xvals[qv[1]]),"l");
  leg->AddEntry(m,TString::Format("Median Sensitivity = %1.3f",xvals[qv[2]]),"l");
  leg->Draw();
  c->Update();
  ...
  /*   senspath.cd(universepath); */
  /*   TString graph = TString::Format("sens-contour_%i", i); */
  /*   TGraph* sgr = nullptr; */
/*   s.GetObject(graph, sgr)22; */
  /*   TGraph temp(sgr->GetN(), sgr->GetY(), sgr->GetX()); */
  /*   double x = temp.Eval(dm2_val); */
  /*   if (x>1 || x<0.01) { */
